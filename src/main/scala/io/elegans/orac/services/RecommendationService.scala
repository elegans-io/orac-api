...
object RecommendationService {
  ...
  def read(index_name: String, ids: List[String]): Future[Option[Recommendations]] = Future {
    val client: TransportClient = elastic_client.get_client()
    ...
    val documents : List[Recommendation] = response.getResponses
      ...
      val document = Recommendation(id = Option { id }, name = name, user_id = user_id, item_id = item_id,
        generation_timestamp = generation_timestamp, score = score)
      document
    })

    Option{ Recommendations(items = documents) }
  }

  def getUserRecommendations(index_name: String, id: String, from: Int = 0, to: Int = 10):
  Future[Option[Recommendations]] = Future {
    ...
    val documents : List[Recommendation] = search_response.getHits.getHits.toList.map({ case (e) =>
      ...
      Recommendation(id = Option{id}, user_id = user_id, item_id = item_id, name = name,
        generation_timestamp = generation_timestamp, score = score)
    })
