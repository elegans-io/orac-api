...
object IndexJsonOracUsers extends OracApiJsonSupport {
  ...
  private[this] case class Params(
                                   ...
                                   inputfile: String = "./orac_users.json",
                                   separator: Char = '\t',
                                   timeout: Int = 60,
  ...
  private[this] def loadData(params: Params): Iterator[OracUser] = {
    Source.fromFile(params.inputfile).getLines.map { case (line) =>
      Try(Await.result(Unmarshal(line).to[OracUser], 10.second)) match {
        case Success(oracUser) => Some(oracUser)
        case Failure(e) =>
          println("Error unmarshalling oracUser(" + line + "): " + e.getMessage)
          None
      }
  ...
  def main(args: Array[String]) : Unit = {
    ...
    val parser = new OptionParser[Params]("IndexOracUsers") {
      head("Index oracUsers")
      ...
      opt[String]("inputfile")
        .text(s"path of the file with oracUser to index" +
