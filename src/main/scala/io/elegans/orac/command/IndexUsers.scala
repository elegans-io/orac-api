...
import io.elegans.orac.entities.{NumericalProperties, OracProperties, OracUser}
...
object IndexUsers extends OracApiJsonSupport {
  ...
  private[this] def loadData(params: Params): Iterator[OracUser] = {
    ...
    val userIterator = itemsEntries.zipWithIndex.map(entry => {
      ...
      val id = user("user")
      val numericalProperties = user.filter(_._1 != "user").map(e => {
        val key = e._1
