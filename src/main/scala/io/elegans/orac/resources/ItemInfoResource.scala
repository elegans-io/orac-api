              ...
              extractMethod { method =>
                parameters("refresh".as[Int] ? 0) { refresh =>
                  entity(as[ItemInfo]) { document =>
                    ...
                    onCompleteWithBreaker(breaker)(itemInfoService.create(index_name, document, refresh)) {
                      case Success(t) =>
              ...
              authorizeAsync(_ =>
                ...
                extractMethod { method =>
                  parameters("id".as[String].*) { id =>
                    val breaker: CircuitBreaker = OracCircuitBreaker.getCircuitBreaker()
                    onCompleteWithBreaker(breaker)(itemInfoService.read(index_name, id.toList)) {
                      case Success(t) =>
              ...
              authorizeAsync(_ =>
                ...
                extractMethod { method =>
                  entity(as[UpdateItemInfo]) { update =>
                    parameters("refresh".as[Int] ? 0) { refresh =>
                      val breaker: CircuitBreaker = OracCircuitBreaker.getCircuitBreaker()
                      onCompleteWithBreaker(breaker)(itemInfoService.update(index_name, id, update, refresh)) {
                        case Success(t) =>
                          completeResponse(StatusCodes.OK, StatusCodes.BadRequest, Option {
                            t
                          })
              ...
              authenticateBasicAsync(realm = auth_realm,
                ...
                authorizeAsync(_ =>
                  ...
                  extractMethod { method =>
                    parameters("refresh".as[Int] ? 0) { refresh =>
                      val breaker: CircuitBreaker = OracCircuitBreaker.getCircuitBreaker()
                      onCompleteWithBreaker(breaker)(itemInfoService.delete(index_name, id, refresh)) {
                        case Success(t) =>
                          if (t.isDefined) {
